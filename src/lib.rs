//! Official Rust Client for EU CBAM & Carbon Tax Calculation Engine
...
impl CbamCarbonClient {
    pub fn new(api_key: Option<String>, base_url: Option<String>) -> Self {
        let key = api_key
            .or_else(|| env::var("STANZA_API_KEY").ok())
            .or_else(|| env::var("API_KEY").ok())
            .unwrap_or_default();
        let base = base_url.unwrap_or_else(|| "https://api.stanzaapi.com/cbam-carbon".to_string());
        ...
        Self {
            ...
            tool_url: "https://stanzaapi.com/tools/cbam-carbon",
        }
    }
    ...
    pub async fn validate<T: for<'de> Deserialize<'de>>(&self, payload: serde_json::Value) -> Result<ApiResponse<T>, reqwest::Error> {
        self.send_request("/api/v1/calculate", reqwest::Method::POST, Some(payload)).await
    }
